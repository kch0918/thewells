...
function getList(){
	$.ajax({
		type : "POST", 
		url : "/admin/partners/getPtnrCate",
		dataType : "text",
		async : false,
		error : function() 
		{
		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				...
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += 	'	<div id="dep_'+result.list[i].idx+'_eng">'+result.list[i].cate_nm_en+'</div>';
					inner += '	</div>';
					inner += '</div>'; 
					...
					inner2 +='	<div><span class="tit">영문 카테고리명</span> <input type="text" id="cate_en_'+result.list[i].idx+'" value="'+result.list[i].cate_nm_en+'" class="cate_en notEmpty" placeholder="카테고리명을 입력해주세요 " data-name="영문 카테고리명" onkeyup="javascript:cateTyping2(\''+result.list[i].idx+'\');"></div>';
					...
					inner2 += '		<input type="button" value="카테고리 삭제" class="del_btn" onclick="delPtnrCate(\''+result.list[i].idx+'\')">';
					inner2 += '	</div>';
					inner2 +='</div>';
...
//카테고리 추가
function insPtnrCate()
{
	$.ajax({
		type : "POST", 
		url : "/admin/partners/insPtnrCate",
		dataType : "text",
		async : false,
		data : 
		{
...
//카테고리 삭제
function delPtnrCate(idx)
{
	var cnt = $("#cnt_"+idx+"").text();

	if(cnt > 0)
	{
		alert("카테고리 내 게시글이 1개 이상일 시 삭제가 불가합니다.");
	}
	else
	{
		if(confirm("정말 삭제하시겠습니까?")){
			$.ajax({
				type : "POST", 
				url : "/admin/partners/delPtnrCate",
				dataType : "text",
				async:false,
				data : 
				{
...
function fncSubmit()
{
	...
	for (var i = 0; i < A.length; i++) {
		...
		fin_cate_eng +=  $("#cate_en_"+A[i].idx).val() + "|";
	...
	if(validationFlag == "Y")
	{
		$.ajax({
			type : "POST", 
			url : "/admin/partners/saveCate",
			dataType : "text",
			data : 
			{
				idx 	: fin_idx,
				cate_kr : fin_cate_kor,
				cate_en : fin_cate_eng,
				is_show : fin_show_yn, 
				sort 	: fin_sort
			},
			error : function() 
...
function save_pr_cate(){
	...
	$('.order_no').each(function(){ 
  	...
  	$.ajax({
		type : "POST", 
		url : "/admin/partners/upOrder_cate",
		dataType : "text",
		data : 
		{
			idx_arr : idx_arr,
...
<div id="container" class="sub_page part_cate">
	<div class="top_cont">
		<div class="info_txt">
			<ul>
				<li class="inb"><a href="/admin/partners/partners_cate">Partners</a></li>
				<span class="divider">></span>
				<li class="inb"><a href="/admin/partners/partners_cate">카테고리 관리</a></li>
			</ul>
			    ...
			    <div class="cate-left inb">
			    	<div class="category-btn">
				         <ul>
				            ...
				            <li class="inb"><button onclick="javascript:insPtnrCate();" class="small-btn"></button></li>
				        </ul>
				    </div>
