		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '<p class="name">'+result.list[i].team_nm_en+'</p>';
					inner += '<p class="team-en">'+result.list[i].level_en+'</p>';
	...
	$.ajax({
		...
		success : function(data) 
		{
			...
			{	
				...
				{
					$('#tag_team_nm_ko').text("#" + result.data.intro_en);
				}
				...
				$('#team_nm_ko').text(result.data.team_nm_en);
				$('#level_en').text(result.data.level_en);
				$('#level_ko').text(result.data.cate_nm);
				
				
				var arr = result.data.career_en.split('<br>');
				...
				var arr2 = result.data.edu_en.split('<br>');
	...
	<div class="inner">
		...
		<div class="sub-cont">
			<div class="sub-cont-inner">
				<div class="nav-wr">
				         <ul class="nav">
						    <li class="nav-it ko n1"><a href="all_en" class="target_0" onclick="goDetail('0')">ALL<span class="count">${All_cnt}</span></a></li>
						    <c:forEach items="${team_cate}" var="i">
				         		<li class="nav-it ko n1"><a href="/team/all_en?cate_idx=${i.idx}" class="target_${i.idx}" onclick="goDetail('${i.idx}')">${i.cate_nm_en}<span class="count">${i.cate_cnt}</span></a></li>
						    </c:forEach>
						</ul>
				</div>
	...
	<div class="inner fz0 d">
		...
		<div class="right-wr scroll">
			...
			<div class="career-box dot">
				<p class="tit n-d">Work Exerience</p>
				<p id="career_ko" class="n-d"/>
			</div>
			<div class="educa-box dot">
				<p class="tit n-d">Education</p>
